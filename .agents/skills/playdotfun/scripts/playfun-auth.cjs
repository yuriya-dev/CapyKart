...
function checkStatus() {
  ...
  if (!creds) {
    ...
    console.log('\nRun "node playfun-auth.cjs setup" to configure credentials.\n');
    return false;
  }
...
Usage:
  node playfun-auth.cjs <command> [options]
...
Examples:
  node playfun-auth.cjs status
  node playfun-auth.cjs callback
  node playfun-auth.cjs manual YXBpLWtleTpzZWNyZXQta2V5
  node playfun-auth.cjs clear
  node playfun-auth.cjs setup
      `);
      break;
  }
}
